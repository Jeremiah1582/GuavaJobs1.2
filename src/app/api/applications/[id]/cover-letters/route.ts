...
import { coverLettersService } from "@/lib/applications/server"
...
export const GET = withErrorHandler(async (_request, context) => {
  ...
  try {
    const { letter } = await coverLettersService.getLetterPayload(session.id, id)
    return jsonSuccess({ letter, manual: letter })
  } catch (err) {
